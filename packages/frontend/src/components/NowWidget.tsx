  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0">
        <div>
          <CardTitle className="flex items-center gap-2">
            Now in Warsaw
            {w && <Badge variant="outline" className="font-normal text-xs">{weatherLabel(w.weatherCode)}</Badge>}
          </CardTitle>
          {w && <p className="text-xs text-muted-foreground mt-1">Captured at {fmtDate(w.capturedAt)}</p>}
        </div>
        {user?.role === 'admin' && (
      ...
      <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-6">
        <Metric label="Temperature" value={`${fmtNum(w?.temperature ?? null, 1)}°C`} />
        <Metric label="Precipitation" value={`${fmtNum(w?.precipitation ?? null, 1)} mm`} />
        <Metric label="Cloud cover" value={`${fmtNum(w?.cloudCover ?? null, 0)}%`} />
        <Metric label="Sunshine" value={`${fmtNum((w?.sunshineLastHour ?? 0) / 60, 1)} min`} />
        ...
        <Metric label="Loudness" value={`${fmtNum(m.avgLoudness, 1)} dB`} accent />
        ...
        <Metric label="Instrumental" value={fmtNum(m.avgInstrumentalness)} accent />
      </CardContent>
    </Card>
  );
}
